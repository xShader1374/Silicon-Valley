...
func handle_double_tap():
	if state == 'crouching':
		return
	for m_action in [controls.FORWARD, controls.LEFT,  controls.BACKWARD, controls.RIGHT]: # tutti le possibili action di movimento, dando priorità a quella già premuta c[]
...
func handle_jumping():
	...
	if Input.is_action_just_pressed(controls.JUMP) and (is_on_floor() or (DOUBLE_JUMP_ENABLED and can_do_another_jump)) and !low_ceiling:
		var multiply = 1
		if not(is_on_floor()) and can_do_another_jump:
